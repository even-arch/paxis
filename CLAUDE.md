...
無論從哪個管道進來（手動、AI 掃描、Patisco MCP），**外部給的單號一律不得由系統修改**。

```
客戶 PO 號   →  SLS_Order.orderNo   （沿用客戶自己的單號）
採購單號     →  PO_Order.poNo       （通常與客戶 PO 號相同，拆單時加後綴 -1/-2/-3）
供應商 PI 號 →  PO_SupplierPI.piNo  （供應商自定，保留原值）
我方 PI 號   →  SLS_PI.piNo         （需引用 SLS_Order.orderNo，讓客戶對應）
```
