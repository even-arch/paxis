...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  const order = await prisma.pO_CustomerCopy.findUnique({ where: { id: orderId } })
  if (!order) return NextResponse.json({ error: 'Not found' }, { status: 404 })

  const body = await req.json()
  const item = await prisma.pO_CustomerCopy_Item.create({
    data: {
      orderId,
...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  const item = await prisma.pO_CustomerCopy_Item.findUnique({ where: { id: itemId } })
  if (!item || item.orderId !== orderId)
    return NextResponse.json({ error: '品項不屬於此訂單' }, { status: 400 })

  const body = await req.json()
  const updated = await prisma.pO_CustomerCopy_Item.update({
    where: { id: itemId },
    data: {
...
export async function DELETE(req: NextRequest, { params }: Params) {
  ...
  const item = await prisma.pO_CustomerCopy_Item.findUnique({
    where: { id: itemId },
  ...
  if (item.piItems.length > 0) {
    await prisma.pI_Item.deleteMany({ where: { slsItemId: itemId } })
  }

  await prisma.pO_CustomerCopy_Item.delete({ where: { id: itemId } })
