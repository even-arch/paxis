    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          ...
          {!showArchived && (
            <Link href="/suppliers/new" className="bg-blue-600 text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-blue-700">
              + 新增供應商
            </Link>
          )}
        </div>
      ...
      <form method="GET" className="mb-4 flex gap-2">
        ...
        {search && (
          <Link href={showArchived ? '/suppliers?archived=true' : '/suppliers'}
            className="border border-gray-300 px-4 py-2 rounded-md text-sm hover:bg-gray-50 text-gray-500">清除</Link>
        )}
      </form>

      <SupplierListClient suppliers={suppliers} isArchived={showArchived} />
        ...
        <div className="flex items-center gap-2 mt-4 text-sm">
          ...
          <div className="flex gap-1 ml-auto">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => {
              ...
              return (
                <Link key={p} href={`/suppliers?${pp.toString()}`}
