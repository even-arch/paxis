...
const VALID_SORTS = ['name', 'sku', 'modelNo', 'unit', 'createdAt', 'stock'] as const
...
export default async function ProductsPage({ searchParams }: Props) {
  ...
  // stock 是計算欄位，需先取全部再排序分頁
  const isStockSort = sort === 'stock'
  const [total, rawProducts] = await Promise.all([
    prisma.pRD_Product.count({ where }),
    prisma.pRD_Product.findMany({
      where,
      orderBy: isStockSort ? { sku: 'asc' } : { [sort]: dir },
      skip: isStockSort ? 0 : (page - 1) * limit,
      take: isStockSort ? undefined : limit,
      include: { inventoryItems: { select: { quantity: true } } },
    }),
  ])

  const products = isStockSort
    ? rawProducts
        .map(p => ({ ...p, _stock: Number(p.inventoryItems[0]?.quantity ?? 0) }))
        .sort((a, b) => dir === 'asc' ? a._stock - b._stock : b._stock - a._stock)
