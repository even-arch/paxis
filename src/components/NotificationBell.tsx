      ...
      {open && (
        <div className="absolute left-0 top-10 w-80 bg-white rounded-lg shadow-xl border border-gray-200 z-50 text-gray-800 text-sm">
          <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
            ...
            {hasAlerts && (
              <span className="text-xs text-gray-400">{alerts.length} 張出貨單</span>
            )}
          </div>
          ...
          {hasAlerts && (
            <>
              <ul className="max-h-52 overflow-y-auto divide-y divide-gray-50">
                {alerts.map(a => (
                  <li key={a.shipmentId} className="px-4 py-2.5">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {a.grossPct != null && (
                        <span className={`ml-auto text-xs font-mono ${a.grossPct > 55 || a.grossPct < 0 ? 'text-red-500' : 'text-gray-400'}`}>
